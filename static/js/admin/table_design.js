      ...
      success: function (response) {
        if (response.status === 200 && Array.isArray(response.data)) {
          response.data.forEach(t => {
            const cell = $(`.table-grid [data-value='${t.table_code}']`);
...
$(document).on('click', '.table-grid > div', function () {
    ...
    const tableCode = $(this).data('value');

    // Check if walk-in
    if (data.is_walkin) {
        Swal.fire({
            title: 'Set Table Available?',
            text: `Do you want to set "${tableName}" as Available?`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    method: "POST",
                    data: {
                        requestType: "set_table_available_from_walkin",
                    ...
                    success: function (response) {
                        if (response.status === "success") {
                            Swal.fire(
                                'Updated!',
                                `"${tableName}" is now Available.`,
                                'success'
                            );
                            refreshData(); // Refresh the table grid
                        } else {
                            Swal.fire(
                                'Error!',
...
// Handle Set Schedule click for Walk-in tables
$(document).on('click', '.setSchedule', function (e) {
    e.preventDefault();
    const cell = $(this);
    const tableName = cell.find('.table-name').text().trim();
    const tableCode = cell.data('value'); // make sure your cell has data-value attribute

    Swal.fire({
        title: 'Set Not Available?',
        text: `Do you want to set "${tableName}" as Not Available?`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                method: "POST",
                data: { 
                    requestType: "set_table_unavailable_walking",
                    table_code: tableCode
                },
                dataType: "json",
                success: function (response) {
                    if (response.status === "success") {
                        Swal.fire(
                            'Updated!',
                            `"${tableName}" is now Not Available.`,
                            'success'
                        );
                        refreshData(); // Refresh your table grid
                    } else {
                        Swal.fire(
                            'Error!',
